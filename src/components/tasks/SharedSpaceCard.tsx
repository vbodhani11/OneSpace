...
export function SharedSpaceCard({ space, onClick, onDelete }: SharedSpaceCardProps) {
  ...
  return (
    <motion.div
      ...
      className="glass-card p-2 hover:bg-white/8 transition-all duration-200 group"
    >
      <div className="flex items-start gap-3">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {isOwner && <Crown aria-label="Owner" size={12} className="text-yellow-400 flex-shrink-0" />}
            </div>
            {space.description && (
        ...
        {isOwner && onDelete && (
          <button
            ...
            className="m-2 p-2 text-white/40 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all"
          >
            <Trash2 size={15} />
          </button>
        )}
